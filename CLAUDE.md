...
- **Signup form** (`#signupForm` in `web/index.html`) POSTs JSON to the URL in its `action` attribute. The current value `https://YOUR-API-GATEWAY-URL.amazonaws.com/prod/signup` is a **placeholder** — replace it with the real API Gateway endpoint before shipping. The form includes a `_gotcha` honeypot field; don't remove it.1
