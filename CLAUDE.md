...
- **`SiteApi`** (`iac/stacks/resources/api.py`) is a REST API Gateway that fronts both Lambdas at `GET /prod/altcha` and `POST /prod/signup` with CORS preflight handled by API Gateway itself. After `cdk deploy`, the URLs are emitted as CFN outputs (`ApiBaseUrl`, `AltchaChallengeUrl`, `SignupUrl`); copy them into the form `action` and `<altcha-widget challengeurl=…>` in `web/index.html`, then re-run `cdk deploy` to push the updated HTML. CORS allowlist is configurable via `api.allowedOrigins` (defaults to any origin).
